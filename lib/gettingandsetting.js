...
export const getCommentsByStateName = async (stateName) => {
  const res = await fetch(`/api/states/comments/${stateName}`);
  const data = await res.json();
  if (!res.ok) {
    return { status: "error", data };
  }
...
export const addComment = async (commentData, stateName) => {
  const res = await fetch(`/api/states/comments/${stateName}`, {
    method: "POST",
